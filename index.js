...
function joinRtm () {
  ...
  rtc.clientRtm = AgoraRTM.createInstance(options.appid,{enableLogUpload: true, logFilter: AgoraRTM.LOG_FILTER_DEBUG });
...
function sendChannelMessage(localMessage){
  ...
  rtc.channelRtm.sendMessage({text:$("#messageDisp").val()}).then(function(){
  //rtc.channelRtm.sendMessage({text:localMessage+$("#messageDisp").val()}).then(function(){
...
function setDispMessage(localMessage){
  ...
  //$("#messageDisp").val(currentMessage + "\n")
}
